...
export async function importMermaid(source, options = {}) {
    ...
    const feedbackEdgeIds = graph.kind === 'flowchart'
...
function parseFlowchart(source) {
    ...
    if (!header)
        throw new Error('无法识别 flowchart 方向');
    const nodes = new Map();
    const edges = [];
    ...
    lines.forEach((line) => {
        ...
        if (classAssignment) {
            classAssignment[1].split(',').map((id) => id.trim()).filter(Boolean).forEach((id) => {
                ...
                if (current)
                    nodes.set(id, { ...current, classes: nextClasses, tone: resolveTone(nextClasses) });
            });
            return;
        }
    ...
    classes.forEach((nodeClasses, id) => {
        const current = nodes.get(id);
        if (current)
            nodes.set(id, { ...current, classes: nodeClasses, tone: resolveTone(nodeClasses) });
    });
    if (nodes.size === 0)
        throw new Error('flowchart 中没有可渲染节点');
    return {
...
function parseFlowNode(token) {
    const value = token.trim();
    const match = value.match(/^([\p{L}\p{N}_:-]+)(.*)$/u);
    if (!match)
        return { id: value, label: normalizeLabel(value), shape: 'rect' };
    ...
    for (const form of forms) {
        ...
        if (label !== undefined)
            return { id, label: normalizeLabel(label), shape: form.shape };
    }
...
function parseSequence(source) {
    ...
    if (entries.length === 0)
        throw new Error('sequenceDiagram 中没有参与者或消息');
    const nodeIds = new Map(entries.map(([id]) => [id, `actor:${id}`]));
    return {
        direction: 'LR',
        edges: messages.map((message, index) => createEdge('sequence', index, nodeIds.get(message.from), nodeIds.get(message.to), `${index + 1}. ${message.label}`, { stroke: message.arrow.startsWith('--') ? 'dotted' : 'normal' })),
        kind: 'sequence',
...
function parseState(source) {
    ...
    sourceLines(source).slice(1).forEach((rawLine) => {
        ...
        if (declaration)
            aliases.set(declaration[1], declaration[1]);
    ...
    if (names.length === 0)
        throw new Error('stateDiagram 中没有状态');
    ...
    if (usesStart)
        nodes.unshift(createNode('state:__start', '开始', nodes.length, 'circle', 'neutral'));
    if (usesEnd)
        nodes.push(createNode('state:__end', '结束', nodes.length, 'circle', 'neutral'));
    return {
        direction: 'LR',
        edges: relations.map((relation, index) => createEdge('state', index, relation.from === '[*]' ? 'state:__start' : nodeIds.get(relation.from), relation.to === '[*]' ? 'state:__end' : nodeIds.get(relation.to), relation.label ?? '')),
        kind: 'state',
        nodes,
    };
}
function parseEntities(kind, source) {
    ...
    sourceLines(source).slice(1).forEach((rawLine) => {
        ...
        if (declaration) {
            if (!entities.has(declaration[1]))
                entities.set(declaration[1], { name: declaration[1], fields: [] });
            return;
        }
        relationLines.push(line);
    });
    const relations = relationLines.map((line) => parseRelation(line, [...entities.keys()])).filter((value) => value !== null);
    ...
    if (entries.length === 0)
        throw new Error(`${kind === 'class' ? 'classDiagram' : 'erDiagram'} 中没有实体`);
...
function parseRelation(line, knownNames) {
...
function parseGantt(source) {
    ...
    sourceLines(source).slice(1).forEach((rawLine) => {
        ...
        if (!line.includes(':') || /^(?:title|dateFormat|axisFormat|excludes|todayMarker)\b/iu.test(line))
            return;
    ...
    if (tasks.length === 0)
        throw new Error('gantt 中没有任务');
    const nodeIds = tasks.map((task, index) => `task:${task.sourceId ?? index}`);
    const sourceIds = new Map(tasks.map((task, index) => [task.sourceId, nodeIds[index]]).filter((entry) => entry[0] !== undefined));
    const nodes = tasks.map((task, index) => createNode(nodeIds[index], [task.label, [task.date, task.duration].filter(Boolean).join(' · ')].filter(Boolean).join('\n'), index, 'round', task.status.includes('done') ? 'green' : task.status.includes('crit') ? 'orange' : task.status.includes('active') ? 'blue' : undefined));
    return {
        direction: 'LR',
        edges: tasks.slice(1).map((task, index) => createEdge('gantt', index, task.after && sourceIds.has(task.after) ? sourceIds.get(task.after) : nodeIds[index], nodeIds[index + 1], task.after ? '依赖' : '')),
        kind: 'gantt',
        nodes,
    };
}
function parseGitGraph(source) {
    ...
    const branchHeads = new Map([[currentBranch, undefined]]);
    const branchTones = new Map([[currentBranch, 'blue']]);
    const commits = new Map();
    const nodes = [];
    const edges = [];
    const toneForBranch = (branch) => {
    ...
    const appendCommit = (label, sourceCommit, stroke = 'normal') => {
        ...
        if (previousHead)
            edges.push(createEdge('git', edges.length, previousHead, id, '', { stroke }));
    ...
    sourceLines(source).slice(1).forEach((rawLine) => {
        ...
        if (checkout) {
            ...
            if (!branchHeads.has(currentBranch))
                branchHeads.set(currentBranch, undefined);
    ...
    if (nodes.length === 0)
        throw new Error('gitGraph 中没有提交');
...
function parseTimeline(source) {
    ...
    sourceLines(source).slice(1).forEach((rawLine) => {
        const line = rawLine.trim();
        if (/^title\s+/iu.test(line) || !line.includes(':'))
            return;
        const [date, text = ''] = line.split(/:(.+)/u);
        if (date.trim())
            currentDate = date.trim();
        if (text.trim())
            events.push({ date: currentDate, text: text.trim() });
    });
    if (events.length === 0)
        throw new Error('timeline 中没有事件');
...
function parseMindmap(source) {
    ...
    entries.forEach((entry, index) => {
        ...
        if (parent)
            edges.push(createEdge('mindmap', edges.length, parent.id, id, ''));
...
function createNode(id, label, index, shape, tone) {
    return { classes: [], id, label: normalizeLabel(label), shape, tone: tone ?? diagramTones[index % diagramTones.length] };
}
function createEdge(prefix, index, sourceId, targetId, label, options = {}) {
    return {
...
function readGitOption(value, name) {
