...
function layoutDiagramGraph(graph, patches, boardLayout, measuredEdgeLabels = new Map()) {
    ...
    const crossCenters = orderAndAlignRankCross(groups, sortedRanks, flowEdges.filter((edge) => !edge.manual), horizontal, nodeGap, groupOrderByNode);
    ...
    const minimumCross = crossBounds.length > 0 ? Math.min(...crossBounds.map(({ low }) => low)) : 0;
    const maximumCross = crossBounds.length > 0 ? Math.max(...crossBounds.map(({ high }) => high)) : 1;
    ...
    sortedRanks.forEach((rank, rankIndex) => {
        ...
        group.forEach((node) => {
            const crossCenter = crossMargin + ((crossCenters.get(node.id) ?? 0) - minimumCross);
 ...
 */
function orderAndAlignRankCross(groups, sortedRanks, edges, horizontal, nodeGap, groupOrderByNode) {
    const crossSizeOf = (node) => horizontal ? node.height : node.width;
    const rankIndexById = new Map();
    sortedRanks.forEach((rank, index) => {
        (groups.get(rank) ?? []).forEach((node) => rankIndexById.set(node.id, index));
    });
    const neighborsAbove = new Map();
    const neighborsBelow = new Map();
    edges.forEach((edge) => {
        ...
        const [upper, lower] = sourceIndex < targetIndex
    ...
    const packCentered = (rank) => {
        const group = groups.get(rank) ?? [];
        const total = group.reduce((sum, node) => sum + crossSizeOf(node), 0) +
    ...
    const neighborMean = (nodeId, neighbors) => {
        const linked = neighbors.get(nodeId) ?? [];
        if (linked.length === 0)
            return undefined;
        return (linked.reduce((sum, id) => sum + (centers.get(id) ?? 0), 0) / linked.length);
    };
    const sortRank = (rank, neighbors) => {
        const group = groups.get(rank) ?? [];
        if (group.length < 2)
            return;
        const keys = new Map(group.map((node) => [
            node.id,
    ...
    const alignRank = (rank, neighbors) => {
        const group = groups.get(rank) ?? [];
        if (group.length === 0)
            return;
        const desired = group.map((node) => neighborMean(node.id, neighbors) ?? centers.get(node.id) ?? 0);
        const placed = packOrderedCrossCenters(group.map(crossSizeOf), desired, nodeGap);
 ...
 */
function packOrderedCrossCenters(sizes, desired, gap) {
    const clusters = [];
    sizes.forEach((size, index) => {
        ...
        while (clusters.length > 0) {
            ...
            if (previous.sum / previous.count + shift <= cluster.sum / cluster.count)
                break;
            clusters.pop();
            cluster = {
...
function resolveAnchorSides(source, target, direction = 'LR') {
