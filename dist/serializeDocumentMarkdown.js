export function serializeDocBlock(block) {
    switch (block.type) {
        case 'heading':
        ...
        case 'table':
            return [
                ...
                ...block.rows.map((row) => `| ${block.headers.map((_, index) => escapeMarkdownTableCell(row[index] ?? '')).join(' | ')} |`),
            ];
        case 'timeline':
...
//# sourceMappingURL=serializeDocumentMarkdown.js.map