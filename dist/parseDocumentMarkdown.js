    ...
    while (cursor < lines.length) {
        ...
        if (image) {
            ...
            if (images.length >= 2) {
                ...
                blocks.push(...images.map((item) => ({ type: 'image', ...item })));
            }
            continue;
        }
...
function parseTable(lines, index, statusOptions) {
    const rows = [];
    let cursor = index;
    ...
    const normalizedRows = rows.map((row, rowIndex) => {
        if (rowIndex === 0 || row.length === headerWidth)
            return row;
...
function parseStatusOptionsComment(line) {
...
function parseAnnotationQuote(quoteLines) {
...
function parseFormula(lines, index) {
    const latexLines = [];
    let cursor = index + 1;
...
//# sourceMappingURL=parseDocumentMarkdown.js.map