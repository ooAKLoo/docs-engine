...
function prefersBuiltInTreeLayout(document: BoardDocument) {
  if (
    document.diagramKind !== 'flowchart'
  ...
  const outgoing = new Map(document.nodes.map(({id}) => [id, [] as string[]]));
  ...
  if (mergeId !== undefined && (outgoing.get(mergeId)?.length ?? 0) > 0) return false;
  const treeEdges = edges.filter(({targetId}) => targetId !== mergeId);
  const treeNodeCount = document.nodes.length - (mergeId === undefined ? 0 : 1);
  if (treeEdges.length !== treeNodeCount - 1) return false;

  const roots = [...incoming].filter(([, count]) => count === 0).map(([id]) => id);
  if (roots.length !== 1) return false;
  if (![...outgoing.values()].some((targets) => targets.length > 1)) return false;
...
function parseFlowEdges(line: string) {
  ...
  const connectors = [...line.matchAll(
    /(<-->|<==>|-->|<--|==>|<==|-\.->|~~~|---)(?:\s*\|([^|]+)\|)?/gu,
  )];
  ...
  for (let index = 0; index < connectors.length; index += 1) {
    ...
    if (connectorMatch.index === undefined || !connectorMatch[0]) return [];
...
function connectorStroke(value: string): ParsedDiagramEdge['stroke'] {
  ...
  if (value.includes('==')) return 'thick';
