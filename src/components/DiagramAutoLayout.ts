 ...
 */
export function measureDiagramTextWidth(value: string, scale = 1) {
  return [...value].reduce((width, character) => {
    const codePoint = character.codePointAt(0) ?? 0;
    const characterWidth =
      codePoint > 0xff ? 14 : character === ' ' ? 4 : /[A-Z0-9]/.test(character) ? 8.2 : 7.2;
    return width + characterWidth * scale;
  }, 0);
}

/** Preserve explicit line breaks, then wrap CJK by character and Latin by word. */
export function wrapDiagramText(value: string, maximumWidth: number, scale = 1) {
  const safeMaximum = Math.max(24, maximumWidth);
  return value.split('\n').flatMap((line) => wrapDiagramLine(line, safeMaximum, scale));
}

export function measureDiagramEdgeLabel(
  value: string,
  bare = false,
  maximumTextWidth = EDGE_LABEL_MAX_TEXT_WIDTH,
): DiagramLabelMetrics {
  if (!value) return {height: 0, lines: [], width: 0};
  const paddingX = bare ? 7 : 9;
  const paddingY = bare ? 3 : 5;
  const lines = wrapDiagramText(
    value,
    Math.min(EDGE_LABEL_MAX_TEXT_WIDTH, Math.max(EDGE_LABEL_MIN_TEXT_WIDTH, maximumTextWidth)),
    EDGE_LABEL_FONT_SCALE,
  );
  const textWidth = Math.max(
    1,
    ...lines.map((line) => measureDiagramTextWidth(line, EDGE_LABEL_FONT_SCALE)),
  );
  return {
...
export function compactDiagramEdgeLabelMetrics(
  metrics: DiagramLabelMetrics,
  bare = false,
): DiagramLabelMetrics {
...
export function calculateAdaptiveRankGaps(
  rankCount: number,
  horizontal: boolean,
  edges: RankedDiagramEdge[],
  baseGap = 106,
) {
  ...
  edges.forEach((edge) => {
    ...
    if (lastRank - firstRank !== 1 || firstRank < 0 || firstRank >= gaps.length) return;
    const label = edge.metrics ?? measureDiagramEdgeLabel(edge.label);
    const requiredGap =
      (horizontal ? label.width : label.height) + (edge.routePadding ?? 30);
...
export function assignDiagramEdgeLanes(edges: PairableDiagramEdge[]) {
  const groups = new Map<string, PairableDiagramEdge[]>();
  edges.forEach((edge) => {
 ...
 */
export function placeDiagramEdgeLabels(
  edges: DiagramCollisionEdge[],
  nodes: DiagramCollisionNode[],
) {
  const placements = new Map<string, DiagramLabelPlacement>();
  ...
  const occupiedLabels: DiagramRectangle[] = [];
  ...
  labelledEdges.forEach((edge) => {
    ...
    occupiedLabels.push(
      expandRectangle(rectangleForLabel(edge, edge.lockedPosition, metrics), 7),
    );
  });
  ...
  labelledEdges
    ...
    .sort(
      (first, second) =>
        second.metrics.width * second.metrics.height - first.metrics.width * first.metrics.height ||
    ...
    .forEach(({edge}) => {
      const ranked = labelCandidates(edge)
        .map((candidate, index) => {
          ...
          return {
            arrowCollisionArea: arrowObstacles.reduce(
              (sum, obstacle) => sum + rectangleIntersectionArea(rectangle, obstacle),
              0,
            ),
            collisionArea: [...obstacles, ...occupiedLabels].reduce(
              (sum, obstacle) => sum + rectangleIntersectionArea(rectangle, obstacle),
              0,
            ),
            candidate,
...
function wrapDiagramLine(value: string, maximumWidth: number, scale: number) {
  if (!value) return [''];
...
type DiagramLabelCandidate = {
  containmentOverflow: number;
...
type ScoredDiagramLabelCandidate = {
  arrowCollisionArea: number;
...
function labelCandidates(edge: DiagramCollisionEdge): DiagramLabelCandidate[] {
  const pathCenter = polylineCenter(edge.points);
  const segments = edge.points.slice(1).flatMap<DiagramSegment>((end, index) => {
    const start = edge.points[index];
    const length = Math.hypot(end.x - start.x, end.y - start.y);
    const horizontal = Math.abs(start.y - end.y) < 0.1;
    const vertical = Math.abs(start.x - end.x) < 0.1;
    if (length < 1 || (!horizontal && !vertical)) return [];
    const startClearance =
      EDGE_LABEL_SEGMENT_CLEARANCE + (index > 0 ? EDGE_CORNER_MAX_RADIUS : 0);
    const endClearance =
      EDGE_LABEL_SEGMENT_CLEARANCE +
  ...
  const candidates = segments.flatMap((segment) =>
    candidatesForSegment(edge, segment, pathCenter),
  );
...
function candidatesForSegment(
  edge: DiagramCollisionEdge,
  segment: DiagramSegment,
  pathCenter: DiagramPoint,
  ...
  const maximumTextWidth = segment.horizontal
    ? Math.min(
        ...
        Math.max(
          ...
          segment.carrierLength - labelPaddingX * 2,
        ),
      )
    : EDGE_LABEL_MAX_TEXT_WIDTH;
    ...
    for (const direction of [-1, 1]) {
      add(
        segment.horizontal
          ...
          : {x: floatingPosition.x + direction * offset, y: floatingPosition.y},
        true,
        floatingMetrics,
      );
    }
...
function positionOnCarrier(
  ...
  if (!segment.horizontal) {
    return {
      x: segment.start.x,
      y: clampToInterval(
        bounds.minimum + (bounds.maximum - bounds.minimum) * ratio,
        bounds.minimum + metrics.height / 2,
        bounds.maximum - metrics.height / 2,
      ),
    };
  }
...
function longitudinalOverflow(segment: DiagramSegment, rectangle: DiagramRectangle) {
  const bounds = carrierBounds(segment);
  if (segment.horizontal) {
    return (
      Math.max(0, bounds.minimum - rectangle.left) +
      Math.max(0, rectangle.right - bounds.maximum)
    );
  }
  return (
    Math.max(0, bounds.minimum - rectangle.top) +
    Math.max(0, rectangle.bottom - bounds.maximum)
  );
}

function carrierBounds(segment: DiagramSegment) {
...
function compareLabelCandidateScores(
  first: ScoredDiagramLabelCandidate,
  second: ScoredDiagramLabelCandidate,
) {
  ...
  return (
    firstHitsArrow - secondHitsArrow ||
...
function terminalArrowObstacle(edge: DiagramCollisionEdge) {
  if (!edge.arrow || edge.points.length < 2) return undefined;
  const tip = edge.points.at(-1);
  const start = edge.points.at(-2);
  if (!tip || !start) return undefined;
  const segmentLength = Math.hypot(tip.x - start.x, tip.y - start.y);
  if (segmentLength < 0.1) return undefined;
  const direction = {
...
function polylineCenter(points: DiagramPoint[]) {
...
function rectangleForNode(node: DiagramCollisionNode): DiagramRectangle {
  return {
...
function horizontalLabelExtents(
  edge: Pick<DiagramCollisionEdge, 'align' | 'bare'>,
  metrics: DiagramLabelMetrics,
) {
...
function edgeMetrics(edge: DiagramCollisionEdge, maximumTextWidth: number) {
...
function sameLines(first: string[], second: string[]) {
  return first.length === second.length && first.every((line, index) => line === second[index]);
}

function clampToInterval(value: number, minimum: number, maximum: number) {
...
function rectangleIntersectionArea(first: DiagramRectangle, second: DiagramRectangle) {
