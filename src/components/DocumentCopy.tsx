...
export type DocumentCopyButtonProps = Omit<
...
function serializeInlineNode(node: Node): string {
  ...
  if (tag === 'em' || tag === 'i') return `_${copy}_`;
  if (tag === 'del' || tag === 's') return `~~${copy}~~`;
  if (tag === 'a') {
...
function serializeTable(element: Element) {
  const table = element as HTMLTableElement;
  const rows = Array.from(table.rows).map((row) =>
    Array.from(row.cells).map((cell) =>
      normalizeInlineWhitespace(serializeInlineChildren(cell))
        .replace(/\|/gu, '\\|')
        .replace(/\n/gu, '<br>'),
    ),
  );
...
function serializeList(element: Element, depth = 0): string {
    ...
    .flatMap((item, index) => {
      ...
      const content = normalizeInlineWhitespace(
        Array.from(item.childNodes)
          ...
          .map((child) =>
            child.nodeType === 1 && (child as Element).tagName.toLowerCase() === 'p'
              ? serializeInlineChildren(child as Element)
...
function serializeBlockNode(node: Node): string {
  if (node.nodeType === 3) return normalizeInlineWhitespace(node.textContent ?? '');
  if (node.nodeType !== 1) return '';
  const element = node as Element;
  if (
    element.matches('.de-diagram') &&
    element.querySelector('[data-de-board-semantic]')
  ) {
    return element.querySelector('[data-de-board-semantic]')?.textContent?.trim() ?? '';
  }
  if (shouldSkipElement(element)) return '';
  if (element.matches('.de-code-block')) return serializeCodeBlock(element);
  const tag = element.tagName.toLowerCase();
  if (/^h[1-6]$/u.test(tag)) {
  ...
  if (tag === 'p') return normalizeInlineWhitespace(serializeInlineChildren(element));
  if (tag === 'pre') return serializeCodeBlock(element);
  ...
  if (tag === 'hr') return '---';
  if (tag === 'img') return serializeInlineNode(element);
  if (tag === 'figure') {
...
export function serializeDocumentToMarkdown(root: HTMLElement) {
...
export function DocumentCopyButton({
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      if (!markdown) throw new Error('Document content is empty');
  ...
  return (
    <button
      aria-label={label}
      className={joinClassNames('de-document-copy', className)}
      data-de-copy-ignore
      data-state={state}
      onClick={handleCopy}
      title="复制为 Markdown，包含图表语义"
      type="button"
      {...props}
    >
      {state === 'copied' ? (
